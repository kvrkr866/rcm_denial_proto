...
def generate_analysis_report_pdf(state, output_path: Path) -> Path:
    ...
    try:
        ...
        story.append(Paragraph(f"Revenue Cycle Management — Denial Analysis  |  {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}", body_style))
...
def generate_cover_letter_pdf(state, output_path: Path) -> Path:
    ...
    try:
        ...
        if analysis:
            story.append(Paragraph(f"<b>Assessment Confidence:</b> {analysis.confidence_score:.0%}", body))
        ...
        story.append(Paragraph("Documents Included in This Submission", h2))
        enc_items = ["Cover letter (this document)"]
        if analysis:
            enc_items.append("Claim denial analysis report")
        if state.correction_plan and routing in ("resubmit", "both"):
        ...
        # Documents pending retrieval (TBD placeholders)
        ...
        if evidence and evidence.evidence_gaps:
            for gap in evidence.evidence_gaps:
                pending_items.append(gap)
        elif eob_detail and not (evidence and evidence.evidence_sufficient):
        ...
        if pending_items:
            story.append(Spacer(1, 0.1*inch))
            story.append(Paragraph(
                "Documents Pending Retrieval (to be submitted separately)",
                ParagraphStyle('PendingH2', parent=h2, textColor=colors.HexColor("#E65100")),
            ))
            for i, item in enumerate(pending_items, 1):
                story.append(Paragraph(
                    f"  {i}. TBD: {item}",
                    ParagraphStyle('PendingBody', parent=body, textColor=colors.HexColor("#BF360C")),
                ))
            story.append(Paragraph(
                "Note: The above documents are required for complete adjudication. "
                "They will be submitted as a supplemental package upon retrieval from the provider's records system.",
                small,
            ))
        ...
        story.append(Paragraph(
            ...
            f"Reference: {state.batch_id or 'N/A'} / {state.run_id}  |  "
